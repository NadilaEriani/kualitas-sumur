(() => {
  // cegah init 2x (kalau script.js kepanggil dobel)
  if (window.__SUMUR_MAP_BOOTED__) return;
  window.__SUMUR_MAP_BOOTED__ = true;

  // Leaflet harus ada
  const L = window.L;
  if (!L) {
    console.error(
      "Leaflet (window.L) belum termuat. Pastikan leaflet.js sebelum script.js dan script.js bukan type=module."
    );
    return;
  }
  ...
  async function fetchAllSumurFromSupabase() {
    if (!sb) throw new Error("Supabase SDK belum termuat.");
    ...
    for (let from = 0; ; from += CHUNK) {
      ...
      const { data, error } = await sb
        .from(SUMUR_TABLE)
        .select(cols)
        .order("id_sumur", { ascending: true }) // biar rapi
        .range(from, to);
  ...
  (async function loadSumurFromSupabase() {
    try {
      const data = await fetchAllSumurFromSupabase();

      if (!data.length) {
        hideMapLoading();
        window.AppUI?.toast?.(
          "Data sumur kosong / tidak bisa diakses (cek RLS).",
          false
        );
        return;
      }
      ...
      for (const row of data) {
        ...
        const jenis = String(row.jenis_sumur || "").toLowerCase();
        const isBor = jenis.includes("bor");
        if (isBor) bor++;
        else gali++;

        const septic = Number(row.jarak_septictank_m);
        const selokan = Number(row.jarak_selokan_m);
        ...
        // samakan nama field dengan versi JSON kamu (pH ada di p.pH)
        const p = {
          id_sumur: row.id_sumur ?? row.id ?? "-",
          kecamatan: row.kecamatan ?? "-",
          jenis_sumur: row.jenis_sumur ?? "-",
          pH: row.ph ?? "-", // <-- penting: dari DB kolom "ph"
        ...
        <div class="popup-card">
          ...
          <div class="popup-grid">
            ...
            <div><b>Septic:</b> ${p.jarak_septictank_m ?? "-"} m</div>
            <div><b>Selokan:</b> ${p.jarak_selokan_m ?? "-"} m</div>
      ...
      `);
        ...
        wells.push({
          ...
          props: {
            // tambahin db_id supaya URL focus bisa pakai row.id juga (tidak merusak fitur lama)
            db_id: row.id,
            id_sumur: p.id_sumur,
            kecamatan: p.kecamatan,
            jenis_sumur: p.jenis_sumur,
            pH: p.pH,
            jarak_septictank_m: p.jarak_septictank_m,
            jarak_selokan_m: p.jarak_selokan_m,
            bau: p.bau,
            rasa: p.rasa,
            latitude: lat,
            longitude: lng,
      ...
      hideMapLoading();
      window.AppUI?.toast?.(
        e?.message || "Gagal memuat data sumur dari Supabase",
        false
      );
    }
  })();
  ...
  document
    .getElementById("toggleRiskMedium")
  ...
  function focusFromUrlTarget() {
    ...
    if (id) {
      w =
        wells.find((x) => String(x.props?.id_sumur || "").trim() === id) ||
        null;
    }
  ...
  document
    .getElementById("btn-export-filtered")
